...
Traversals in the above example represents 
following tree 
...
always the first item in preorder traversal and it must be the last item in postorder
 ...
 we search root in in[], all elements before root in in[] are elements of left subtree 
 and all elements after root are elements of right subtree. In pre[], all elements after
  index of root in in[] are elements of right subtree. And elements before index 
  (including the element at index and excluding the first element) are elements of left 
  subtree.

*/

public class PrintPostOrderTraFromGivenPreOrdernInOrderTra {
