...
//SC= o(n) .. o(m+n)
public class findSubStringInAString_KMP_Count {

	public static void main(String[] args) {

		String pattern = "abcaby";
		String text = "abcabcabcaby";
	...
	static void isMatchFound(String text, char patt[], int pattIndex[]) // TC=
																		// 0(n)
	{
		...
		for (i = 0; i < tex.length;) {
			...
			{
				isPresent = true;
				System.out.println("patttern is present1");

				j = 0;

			}

			else if (tex[i] == patt[j]) {
				j++;
				i++;
			} else {
				if (j >= 1)
	...
	static int[] findPatternPrefixIsSuffix(char patt[]) // TC= o(m)
	{
		int j = 0;
